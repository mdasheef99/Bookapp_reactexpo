...
| `src/features/clubs/services/clubsDiscussionService.ts` | service (previous session) | `setClubDiscussionReaction` now calls `supabase.rpc('set_club_discussion_reaction', { in_topic_id, in_reply_id, in_emoji })` (corrected by WU-TC06 2026-09-13; previously misstated here as `p_*`) — no `user_id` argument; canonical row mapping (`id`, `topic_id`, `reply_id`, `user_id`, `emoji`, `created_at`). `removeClubDiscussionReaction` unchanged: retains emoji + actor + exactly-one-target filter. |
