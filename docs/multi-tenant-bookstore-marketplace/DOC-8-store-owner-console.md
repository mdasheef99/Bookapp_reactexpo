...
> **Historical Unit 6H correction checkpoint — 2026-09-12:** The duplicate warning was locally
> integrated into the Owner capture flow at that checkpoint. Dismissal is local-only and leaves a
> visible reopening action; unresolved confirmation cannot be bypassed by Close;
> Proceed and Cancel remain explicit commands. The focused UI checks pass, with
> the existing `VirtualizedList` `act(...)` warning recorded separately. M60 was
> not remotely applied at that checkpoint, then applied during the authorized
> development rollout; M61 is local-only. F-01 is retracted and F-02/F-03 are corrected locally.
> Disposable independent-connection PostgreSQL verification passes; connected
> Edge/Storage remains a rollout gate. This note records evidence and does not
> change the console's normative authorization rules.
...
- explicit Add to Inventory creates one new private inventory row per reviewed
  ...
  existing row
- when the selected coherent edition has no exact cover, review may show one
  separately labelled representative-edition cover from a compatible alternate
  result already returned by the same provider lookup; volume/order and series
  conflict checks must pass, and unresolved series evidence suppresses fallback;
  this never changes the selected metadata or public/canonical cover field
