...
| Field | Value |
|---|---|
| Current phase | Phase 5: Consumer Discovery - **app-side implementation in progress; Phase 3 anonymous public-read RLS remediation remains a backend blocker for public marketplace reads** |
...
| Phase 5: Consumer Discovery | `in_progress` | [PHASE-5](./implementation/PHASE-5-consumer-discovery.md) | App-side consumer discovery has marketplace tab, public-projection service, ISBN-10/ISBN-13/title search, grouped book results with all store offers, store availability cards, public store page, and disclosures. Review follow-up fixed stale search responses, cover rendering, store navigation, availability/condition-note display, and initial/search-submit behavior. Focused marketplace tests, TypeScript, and web export pass. Pending: Phase 3 anon RLS blocker, author search schema gap, return-policy projection gap, and unavailable-search capture decision. |
