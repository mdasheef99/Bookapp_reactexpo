...
| Field | Value |
|---|---|
| Current phase | Phase 5: Consumer Discovery - **app and schema implementation in progress; Phase 3 anonymous public-read RLS remediation remains a backend blocker for public marketplace reads** |
...
| Next recommended task | Resolve the Phase 3 anonymous public listing RLS issue, then run anonymous public-read smoke for Phase 5 consumer discovery. Either explicitly approve the prepared helper-grant migration or implement a narrower policy split that avoids `anon` calling owner/operator helpers. Phase 2C authenticated platform-review smoke remains pending until an explicitly approved platform-role test user is provided. |
...
| Phase 5: Consumer Discovery | `in_progress` | [PHASE-5](./implementation/PHASE-5-consumer-discovery.md) | Consumer discovery has marketplace tab, public-projection service, ISBN-10/ISBN-13/title/author search, grouped book results with all store offers, store availability cards, public store page with return policy, disclosures, and private zero-result demand capture. Live migration `20260701062905 marketplace_phase5_consumer_discovery_schema` was applied through Supabase MCP and verified for columns, indexes, RLS, RPC grants, and rollback-safe demand capture. Focused marketplace tests, migration tests, and TypeScript pass; web export previously passed, but the current rerun was blocked by sandbox `EPERM` and approval-system usage limit. Pending: Phase 3 anon RLS blocker and anonymous public-read smoke. |
...
**Resolve Phase 3 public-read remediation, then run Phase 5 anonymous consumer smoke.**
...
3. Smoke-test Phase 5 consumer discovery against public projections only: `marketplace_book_listings` and `public_store_profiles`, never raw `store_inventory` or P2P `listings`.
