...
| Next recommended task | Run authenticated Phase 3 store-owner smoke with an explicitly approved owner for an active/approved/setup-complete/selling-allowed store. Phase 2C authenticated platform-review smoke remains pending until an explicitly approved platform-role test user is provided. |
...
| Phase 3: Inventory, Canonical Books, and Listings | `in_progress` | [PHASE-3](./implementation/PHASE-3-inventory-canonical-listings.md) | Manual inventory/projection slice implemented, tested, exported, and live-applied; authenticated owner smoke remains. |
...
**Complete authenticated runtime smoke for Phase 3 owner inventory and the pending Phase 2C review flow.**
...
4. Run authenticated Phase 3 inventory smoke against an explicitly approved active seller fixture.
