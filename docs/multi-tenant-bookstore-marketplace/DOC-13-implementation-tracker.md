...
> 2026-07-15 Phase 5 review refresh: Consumer discovery is locally complete after red-test remediation. Public-only services now include runtime validation, safe filter construction, explicit paging/retry, store-name discovery, and book availability detail. Local migration `20260715000002_marketplace_phase5_discovery_hardening.sql` adds pilot-locality policies and a bounded fixed-context rate-limited demand RPC. Relevant Jest 8 suites/54 tests, TypeScript, production web export, and `git diff --check` pass. No Phase 5 hardening migration, function deployment, fixture, or Git operation was performed live.

| Field | Value |
|---|---|
| Current phase | Phase 5: Consumer Discovery - **locally complete after review remediation; live hardening and anonymous/authenticated smoke remain operationally pending** |
...
| Next recommended task | Review and separately authorize the prepared narrow Phase 5 discovery hardening migration, then run anonymous/authenticated consumer smoke with an approved disposable listing fixture. Phase 2C/4 authenticated credential-gated smokes remain pending. |
...
| Phase 5: Consumer Discovery | `in_progress` (`locally_complete`) | [PHASE-5](./implementation/PHASE-5-consumer-discovery.md) | Local review remediation adds runtime validation, safe filters, paging/retry, store-name search, book availability detail, pilot-locality policies, and a bounded fixed-context rate-limited demand RPC. Relevant Jest 8 suites/54 tests, TypeScript, web export, and diff check pass. Live Phase 5 base migration remains applied; local hardening migration is not live. Pending: migration approval and anonymous/authenticated smoke with an approved disposable fixture. |
...
**Review/apply Phase 5 discovery hardening, then run public consumer smoke.**
...
Next work:
1. Review and separately authorize local migrations `20260713000001_marketplace_phase3_public_listing_policy_split.sql` and `20260715000002_marketplace_phase5_discovery_hardening.sql`; neither is live.
2. With separately approved disposable data, rerun projection plus anonymous/authenticated Phase 5 discovery smoke against `marketplace_book_listings` and `public_store_profiles` only, then clean up.
3. Verify the hardened boolean demand RPC accepts only bounded query text, records fixed source/no location, rate-limits abuse, and exposes no private rows or identifiers.
