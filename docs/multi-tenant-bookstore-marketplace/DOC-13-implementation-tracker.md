...
| Next recommended task | Run anonymous/authenticated consumer discovery and bounded demand-capture smoke with an approved disposable listing fixture, then clean it up. Phase 2C/4 authenticated credential-gated smokes remain pending. |
...
| Phase 5: Consumer Discovery | `in_progress` (`locally_complete`) | [PHASE-5](./implementation/PHASE-5-consumer-discovery.md) | Review remediation adds runtime validation, safe filters, paging/retry, store-name search, book availability detail, pilot-locality policies, and a bounded fixed-context rate-limited demand RPC. Relevant Jest 8 suites/54 tests, TypeScript, web export, and diff check pass. The Phase 3 policy split and Phase 5 hardening migrations are live and structurally verified. Pending: anonymous/authenticated positive smoke with an approved disposable fixture. |
...
- Phase 5 discovery hardening deployed (2026-07-15): anonymous/authenticated listing policies split, pilot-locality gates enforced, private inventory/analytics grants verified, and the constrained rate-limited demand RPC plus 90-day retention applied. Positive discovery smoke is fixture-gated.
...
**Run positive Phase 5 public consumer smoke with an approved disposable fixture.**
...
Next work:
1. With separately approved disposable data, rerun projection plus anonymous/authenticated Phase 5 discovery smoke against `marketplace_book_listings` and `public_store_profiles` only, then clean up.
2. Exercise the hardened boolean demand RPC during that smoke to confirm bounded query capture, fixed source/no location, rate limiting, and no private-row or identifier exposure.
3. Run authenticated `store-review` live smoke with a platform-role test user (`platform_admin` or `store_reviewer`) when explicitly approved credentials are available.
4. Review or clean up Phase 2B/2C live smoke fixture rows if desired before production pilot.
5. Keep payment-gated selling, orders, fulfillment, delivery, and Phase 7 commerce out of Phase 5 scope.
