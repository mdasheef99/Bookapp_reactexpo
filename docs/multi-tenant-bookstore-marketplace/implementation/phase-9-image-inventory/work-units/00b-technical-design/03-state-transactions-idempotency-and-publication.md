...
| Publication | Candidate remains `committed`; inventory publication status is `private`, `publication_pending`, `published`, or `publication_failed` | `private|publish|pause` are commands/intents; `committed_publication_failed` is an API outcome only and is never a candidate or publication status |
...
If projection fails after private commit, candidate state remains `committed`, publication status becomes `publication_failed`, and canonical command result is `committed_publication_failed`, mapped through `P9_PUBLICATION_FAILED` HTTP 202 with `private_inventory_committed`. The response exposes inventory ID, current private state, publication status/error code and retry availability—not a false published result or internal error. C12 retries only the projection using the original publication identity and `mayWriteInventory=false`; tests must prove it cannot create/increment inventory.
