...
**Definition status:** `definition_complete_needs_review`
**Date:** 2026-07-19
...
| C02 `authorize_image_upload` | Initiating Owner; session, purpose, source kind, declared file envelope; re-resolve active session/store | Persist one short-lived purpose/entity capability; expected session version; failed issue leaves no upload authority | Auth/state/quota/media errors; `input.upload_authorized`; no URL/token telemetry; store/user/IP rate |
...
| C13 `set_or_get_needs_review` | Initiating Owner for session mutation; same-store authorized Owner for later bounded retrieval | Mark/retrieve staged candidates without inventory write; expected candidate/session version | Auth/state/expiry errors; `candidate.needs_review`; counts/reason codes only |
...
| C15 `provide_request_photos` | Owning store authorized Owner; photo request/version and approved media IDs | Validate request-scoped new media, link 1–3, then mark `provided`; upload alone is insufficient | Media/state/cross-purpose errors; `photos.provided`; IDs/count only |
| C16 `accept_or_decline_request_photos` | Owning request customer; request/version and item decisions | Atomic photo decision plus existing Phase 6 recalculation/hold seam; cannot create payment-provider effects | Customer/stale/missing-media errors; `photos.accepted|declined`; decision/count only |
| C17 `mark_requested_item_unfulfilled` | Owning store authorized Owner; photo request/version/reason code | Mark item unavailable/unfulfilled and invoke existing hold/amount recalculation seam; idempotent | Auth/state/stale errors; `photos.unfulfilled`; bounded reason; no paid-order effect |

The WU0B implementation must decide whether C08–C10 are one externally callable command with internal actions or separate private service operations. It must not multiply public endpoints merely to mirror internal actions.
...
| Q07 marketplace book search | Public/anonymous or authenticated consumer | Safe listing matches; stable query-version cursor; no private inventory fields; public cache keyed by context/version |
| Q08 store-grouped results | Public | One eligible store/group; store-level cursor and final `store_id` tie-break; `bookstore_count`/`offer_count`; no exact quantity |
...
Migration-file creation and live application must never share one authorization. Until independent review approves this definition and the user separately authorizes WU0B implementation, the next action is review only.
