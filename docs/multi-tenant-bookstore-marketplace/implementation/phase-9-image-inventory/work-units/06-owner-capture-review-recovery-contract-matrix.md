...
| Authorize upload | `executeOwnerIngestion(authorize_scan_upload)` → `phase9_issue_scan_upload`; M11/M13 | session/source/MIME/bytes/ordinal/idempotency → capability, signed URL/token, expiry | JWT then service RPC rechecks actor/session; one-time path; no-store | Edge and ingestion integration tests; **adapter** |
| Register media | `executeOwnerIngestion(complete_scan_upload)` → context/object checks → `phase9_register_scan_upload_completion`; M11/M13 | capability/source/idempotency → input/job/state | object identity/hash/size/MIME rechecked; canonical replay; queues sanitation | changed-object/replay tests; **adapter** |
...
| Commit/duplicate mutation/publication | `phase9_commit_candidate`, publication commands; M05 | controlled commit inputs/results | exact auth/version/idempotency/locks | existing backend is outside Unit 6; **U7** |
...
| --- | --- | --- | --- | --- | --- |
| **U6Q01** `discover_scan_session` / `phase9_owner_discover_session_v1` | Discover the caller's one `active|closing` session plus their own cross-session review count. Authenticated eligible initiating Owner only. | `contractVersion`; no store/user/session hint and no cursor. | `activeSession: OwnerSessionLocator|null`; `needsReviewCount:int 0..MAX_SAFE`; `reviewScopeVersion:int>0`. Locator = `sessionId`, `status:active|closing`, `sessionVersion`, `startedAt`, `updatedAt`, `inputCount`, `candidateCount`, `attentionCount`. | Read-only; no key/replay effect. Unique M02 `(store_id,created_by)` active index makes 0/1 deterministic. Count uses `NeedsReviewMembershipV1`; any changed result replaces cache. | `P9_AUTH_REQUIRED|P9_OWNER_NOT_AUTHORIZED|P9_INTERNAL_ERROR`; no other Owner's count/ID. **Migration: yes**, new RPC plus review-scope revision. Red: 0/1 result; count/first-page parity; same-store noninitiator/cross-store/staff/manager/support denied; client `storeId` unknown key; closing returned; two active rows fail invariant. |
...
| `duplicateIntent` | R key with `null` or object `{action:increment_quantity|create_separate|manual_match,targetInventoryId:uuid|null,adviceVersion:int>0}`. Null means no current warning. `increment_quantity|manual_match` require target ID; `create_separate` requires null. | Server requires an allowed action and exact current advice version whenever advice exists; changed advice returns conflict. It persists intent only. `skip` is candidate disposition, not duplicate intent. | Required when advice exists / yes / yes and recompute under locks |
...
| Session `active`, no pending Close | `session_active` / “Scan in progress” | no / while visible work | Add image, add missed, review, summary | none | derived from inputs / derived | Announce on first render only |
...
| Input `queued|processing` with vision job `open|in_progress` | `finding_books` / “Finding books” | no / yes | Leave, add another image | server worker | yes / n/a | Announce once per transition |
...
| Input `ready` | `input_ready` / “Image processed” | yes / no | Review candidates, add image | none | no / n/a | Polite completion announcement |
| Input `skipped` with safe terminal code | `input_skipped` / safe registered label | yes / no | Add clearer image, add missed | Owner chooses new upload | no / n/a | Announce label and next action once |
| Input `failed` with terminal safe code/job `dead_letter|cancelled` | `input_failed` / “Image needs attention” | yes / no | New upload; add missed | Owner, new upload/key | no / n/a | Assertive once; focus Retry guidance |
...
| **6D — Candidate review and strict field editing** | Virtualized session/needs-review cards, rich detail, metadata selected/manual flow, exact field constraints, confirmation, duplicate intent, complete damage disclosure, explicit Save and stale compare/reapply. Excludes false/missed/variant decisions, Close, and commit/publication. | Expected: candidate route and `CandidateCard`, `OriginalFieldEditor`, `ReviewForm`, `ConditionPickerV2`, damage/metadata/attention/duplicate panels, reducers/hooks/tests. Consumes U6Q04/U6Q05/U6C01. Server owns canonical detail/readiness/revisions; TanStack owns fetched snapshots; reducer owns unsaved form/base revisions/key. | Red first: 0/1/15/repeated/partial cards, every §2.3 edge/cross-field, damage conditional focus/private forcing, duplicate inspection, metadata outage/manual path, exact Save and all source-state readiness transitions, metadata/candidate stale compare/reapply, dirty exit, large text/help. Owns AC17-18,20-21,23,29,35. | No migration/deploy. Depends 6A-6C. Complexity **high/domain/a11y**. Independent DTO/form/accessibility review. One merge; no false/missed/variant or U7 adapter. |
