...
**Status:** `unit7b_corrections_pass_ready_for_exact_sha_release_gate`; **last updated:** 2026-08-13
**Unit 6 closure scope:** automatic/functional pipeline PASS; native Unit 6F validation debt deferred `NOT_RUN`/`UNRESOLVED`, not PASS.
**Active work unit:** `unit7b_exact_sha_release_gate`. Finding-001 is `CLOSED`; the bounded latest-subscription-order follow-up is integrated, and the Unit 7B correction status is `UNIT_7B_CORRECTIONS_PASS_READY_FOR_EXACT_SHA_RELEASE_GATE`. Real PostgreSQL and Deno pass on corrected source. Docker container smoke remains `NOT_RUN/BLOCKED_ENVIRONMENT` and `REQUIRED_BEFORE_LIVE`. The unchanged metadata-foundation PGlite fixture and six empty Jest collectors remain explicitly dispositioned debt. M40 is unapplied; exact-project preflight/application and all deployment/live proofs remain `NOT_RUN`.
...
| 7B | [Safe publication/projection after private inventory creation](../work-units/07b-publication-sdd.md) | `corrections_pass_ready_for_exact_sha_release_gate` | run the exact-SHA release gate, then all three deferred `REQUIRED_BEFORE_LIVE` gates on the final post-correction SHA before Stage J |

Unit 7B verification correction (2026-08-12): expanded disposable Unit 7B is
...
| `20260812000040_marketplace_phase9_safe_publication.sql` | not applied; proposed `marketplace_phase9_safe_publication` | no connected exact-project preflight; connected project untouched | local Sol Light correction plus bounded latest-subscription-order follow-up; connected application prohibited | shared rollout eligibility using the latest subscription row, moderation preservation, safe grouped discovery, complete targeted public-media retraction, post-lease exact worker replay, actual cross-layer runtime proof, and prior Unit 7B scope | prior corrected Unit 7B 27/27 baseline; this follow-up correction matrix 6/6; focused migration assertions 4/4 plus Owner-publication/discovery/worker 20/20; cross-layer 4/4 and other prior gates unchanged. M40 remains unapplied and M39 byte-unchanged | `local_follow_up_review_pending` |
