...
> Readback confirms the sidecar, JSONB inventory field, RLS, service-only
...
> the deployed M61 response-contract source matches local. Fresh TypeScript,
> metadata-worker build, deployment-runtime validation, and M61 contract Jest
> 3/3 pass. Metadata worker/client deployment and connected scan/Add remain
> unrun because Render is Git-backed at commit `5731822` while the matching
> runtime is uncommitted. No provider, Storage, inventory, listing, or other
> business-row mutation occurred.

> **2026-09-13 representative-cover rollout closeout:** The bounded follow-up
> decodes Google Books volume evidence, suppresses fallback for either record
> carrying unresolved series evidence, and adds forward-only M62 so standalone
> Owner detail and review-save project M61 provenance. After exact-project and
> SHA-256 preflight, M62 was applied once to `Bookconnect_reactexpo` /
> `ahntbtktjjmvfosgkmgn` as remote version `20260913162154`.
...
> local implementation now keeps the selected coherent metadata edition intact
> and adds a separately labelled compatible-edition cover only when that edition
> has no exact cover. M61 (SHA-256
> `A2A8F9191E29C9813DEBC4E35EA8170F00538288079FFA2E7014621A4D90D463`)
> creates private immutable persistence, a service-only public delegate, Owner
> projection, and a separate private inventory field populated only by explicit
> Add. Public/canonical `cover_url`, M52–M60, and duplicate behavior are
> unchanged. Fresh checks: TypeScript PASS; metadata-worker build PASS; focused
> Jest 132/132; `--detectOpenHandles` groups 72/72 and 55/55; M61 PGlite 2/2;
> existing Unit 6G/7A database cases 45/45. The four investigated structural-
> metadata test defects are now corrected test-only; fresh structural, M55, and
> M61 controls pass 14/14, 7/7, and 2/2. No `act(...)`, open-handle, or force-exit warning reproduced; the
> sole `DEP0040` warning traces to Expo/`whatwg-url-without-unicode`. At that
> checkpoint M61 was not remotely applied; it was applied later as recorded
> above. The Phase 9 continuity/documentation validator passed after registering
> M61 as an unapplied draft, and no external or Git publication
> action occurred.
> [Full evidence](../supporting/unit6h-representative-cover-correction.md).

### Historical 2026-09-13 — structural metadata test-debt correction

- Authorized scope: correct only the four reproduced structural-test defects;
  production code and migration SQL were out of scope and unchanged.
- Red baseline: `phase9StructuralMetadataWorker.integration.test.mjs` reproduced
  10/14 with one pre-M55 identity fixture mismatch, two stale
  `manual_metadata_required` expectations whose actual current outcome was
  `retry_scheduled`, and one duplicate-key cascade after the identity assertion
  bypassed cleanup.
- Correction: the fixture now follows the existing tested M39 and M52–M55
  prerequisite chain, the identity row is removed in `finally`, and the two
  retry assertions match the existing retry contract.
- Fresh verification: structural metadata integration 14/14 PASS; M55 lifecycle/
  identity control 7/7 PASS; M61 representative-cover integration 2/2 PASS. No
  test emitted an `act(...)`, open-handle, force-exit, or Node warning.
- External/Git state at that checkpoint: no Supabase/database/Storage, Render, dispatch, application-
  data, or development-data mutation; no migration application or Git stage,
  commit, push, merge, or deployment. M61 was local/unapplied at that checkpoint
  and was applied later as recorded above.

> **Historical 2026-09-12 Unit 6H bounded correction evidence:** Exact development-project
...
- Implemented deterministic same-response fallback selection requiring exact
  normalized title, full author-set match, compatible base language, and no
  subtitle/series/volume conflict. No second provider call or metadata-field
  stitching is permitted.
- Added M61 private sidecar/service delegate, strict Owner response contracts,
  labelled card/detail rendering, and database-owned copy into the separate
  private inventory field at explicit Add. Public listing cover remains
  unchanged.
- Bounded review correction decodes Google Books volume/order evidence, blocks
  representative fallback for unresolved series evidence, and adds M62 to
  project the same provenance through standalone detail/save and replays.
  M62 changes no table/data/public projection and its private helper is denied
  to every API role.
- Verification and the four unrelated structural-suite failures are recorded in
  the linked evidence document. Representative persistence is best-effort after
  accepted metadata terminalization; a narrow crash/RPC-failure window may leave
  the placeholder, but cannot corrupt selected metadata or public projection.
- M61 is live once from the earlier authorized rollout. M62 was applied once as
  remote version `20260913162154`, with post-apply function/ACL and zero-row
  readback. No Render, worker/client, dispatch, application-data, Storage,
  stage, commit, push, merge, or deploy action occurred in this correction.
- Next gate: separately authorized matching Owner Edge/metadata-worker/client
  deployment and connected no-cover review/Add proof.
...
- Read-only baseline: live cron approximately every minute, metadata dispatcher
  ...
  serial job loop, zero actionable jobs, one historical dead letter, 103 jobs
  over 30 days (102 resolved/one dead letter), 46 successful batch-one
  dispatches, and observed provider latency approximately 0.8–6.7 seconds.
  The deployed provider timeout and Google quota were not readable.
- RED-first coverage proved the old metadata contract rejected 15, HTTP
  telemetry truncated at 10, the worker serialized provider calls, M56 was
  absent, durable retries were mislabeled manual, and a later claim error could
  return before already-claimed work drained.
- Production scope: metadata-only request/telemetry maximum 15; a three-slot
  pool with incremental claims/refill; claim-ordered per-book outcomes;
  in-flight drain on later claim failure; and durable retry-state propagation.
  Generic worker limits remain 10. Every job retains an independent Google
  Books request and unchanged lease/fence/retry/dead-letter/idempotency seams.
- M56 replaces only the private dispatcher definition. Metadata receives 15;
  media, vision, and publication retry remain one. Cron frequency, timeout,
  claim function, queue data, and applied migrations are unchanged.
- Verification completed so far: focused Jest **7 suites / 72 tests**
  (including the metadata-only invoker bound); metadata worker production
  TypeScript build PASS; dispatcher PGlite **30/30**. Documentation continuity,
  diff hygiene, broader related regression,
  and final `.pyc` audit are session-close gates and must be recorded exactly.
- No migration application, deployment, provider call, database/Storage/
  business mutation, staging, commit, push, merge, or PR action occurred.
  Required next order: review → separately authorize commit/worker deployment →
  canary timeout ≤10 seconds and quota/resource/retry/duplicate signals →
  separately authorize M56 application.

### 2026-08-30 — bounded metadata/Add authority correction (M55 applied)

- Scope is limited to selected metadata snapshot identity in compact Add
  preparation, author enforcement only at the final Add/commit boundary while
  saved review remains empty-author compatible, title/author-primary durable
  query identity when bibliographic evidence exists, removal of the hard Google
  language filter, exact-ISBN fallback for ISBN-only queries, and batch-card
  Add advertisement only after full commit eligibility.
- Local forward migration
  `20260830000055_marketplace_phase9_unit6g_metadata_add_authority_correction.sql`
  replaces only internal metadata summary, review-blocker, and batch-card
  function definitions. It performs no DML/backfill and changes no table,
  policy, Storage object, public RPC signature, or grant. It was applied exactly
  once through Supabase MCP as live version
  `20260830084323 marketplace_phase9_unit6g_metadata_add_authority_correction`
  directly after M54.
- Verification: affected-scope Jest **64 passed suites / 690 passed tests**
  with one skipped suite/four skipped tests; M54+M55 PGlite **7/7**; and
  read-only post-apply function-definition verification passed. TypeScript,
  continuity, and `git diff --check` remain session-close gates.
- The local canonical resolver, Add-all design, close audit, older sibling RPC
  compatibility, client/Edge deployment, and fresh live browser Add proof
  against the new contract are outside this bounded pass.

### 2026-08-30 — bounded post-scan Add-all state correction (local)

- Scope is limited to the mounted Unit 6G post-scan commit state. The card's
  authority-conflict state is lifted to the parent; Add-all now uses the same
  authority block together with in-flight IDs and recorded non-busy outcomes
  when computing its exact membership. A candidate that returned
  `no_longer_eligible` can no longer remain in the displayed Add-all count.
  Card status now surfaces committed/non-busy Add outcomes, and bulk retries
  include `needs_attention` in their in-flight state.
- Verification: the affected Unit 6G client regression run passed **6 suites /
  76 tests** with `--forceExit`; `npx.cmd tsc --noEmit` exited 0; the Phase 9
  continuity validator passed; and `git diff --check` passed. The affected UI
  suites retain the pre-existing `VirtualizedList` `act(...)` warnings.
- Boundary: frontend state/presentation and regression tests only. No
  Supabase/RPC/migration/provider/Storage/deployment/browser business mutation,
  staging, commit, push, merge, or PR action occurred. Existing dirty files
  remain preserved.
- Exact next action: independent review/Owner review, then the separately
  gated matching client deployment and live active-session proof.

### 2026-08-30 — direct Add/closeout implementation checkpoint

- Retained the individual strict Save → canonical reread → readiness/action
  check → M39 path and added no separate Submit or automatic commit seam.
- Added a pure strict auto-close policy and mounted hook. It invokes v3 Close
  only for a nonzero fully committed detected set with terminal inputs, zero
  review/processing/removal/false-detection remainder, zero visible cards,
  idle commands, current active authority, online state, and route focus.
- Added explicit manual partial-close warning/copy: successful inventory stays;
  uncommitted candidates become read-only session history; Close performs no
  commit or delete.
- Split count language into image-level input attention versus candidate-level
  book review, resolving the misleading `0 need attention` / `15 need review`
  presentation without changing server count definitions.
- Expanded Unit 7A integration assertions for condition, price, quantity,
  location, damage, publication intent, and private publication preservation.
  Existing exact-replay and overlap tests retain the one-row/one-inventory-ID
  guarantee.
- RED-first focused failures were observed before implementation. Current green
  evidence: full Image Inventory Jest **59 suites / 478 tests**, focused PGlite
  **17/17**, TypeScript, production web export, continuity, and diff hygiene.
- Read-only live preflight found Edge v9 active and one active 12-candidate
  session with zero commits. Connected browser proof selected Book 1's detected
  identity, exposed one enabled individual Add, and pressed it once. Canonical
  response returned inventory `332ad909-47dc-4256-963a-3878c3081337`; readback
  found one row for candidate `d6a18694-a004-437f-95e8-352b64f579e1` with
  condition `good`, ₹175, quantity `1/1/0/0/0`, location `2`, no damage,
  publication decision `publish`, and private/draft state. Reload retained
  Added `1` / Need review `11`; the session stayed active with `closed_at` null.
- Edge v9 already carries all five Unit 6G actions and none of this pass's
  changed files belongs to its deployed bundle. A remote client target is not
  configured: `app.json` has no EAS project ID/owner/updates URL and there is no
  repository web-hosting target. Production export is proven; remote deployment
  remains target-blocked.
- Pending migration-gated work: one exact close actor/source/command/time audit
  plus active/unexpired fences for manual-candidate, skip/false-detection, and
  variant decision/replacement RPCs. No migration was created/applied.
- External state: exactly one authorized inventory business effect plus its
  canonical candidate/idempotency/audit/event/session-count effects; no
  Storage, migration, remote deployment, staging, commit, push, merge, or PR.

### 2026-08-30 — local Google Books matching, cover, and metadata-sheet correction

- Scope was limited to three Owner-authorized tasks. Google Books requests now
  use normalized title plus first author when available. Ranking first requires
  exact normalized title plus author overlap, then treats validated ISBN,
  compatible language, and edition clues as secondary tie-break evidence.
- Cover decoding now checks the full bounded size order (`extraLarge`, `large`,
  `medium`, `small`, `thumbnail`, `smallThumbnail`) and continues past unsafe,
  malformed, or over-bound larger links to the first allowlisted Google Books
  HTTPS URL.
- The on-demand metadata sheet now displays selected cover, subtitle, bounded
  plain-text description, and categories/genre, and replaces generic metadata
  state copy with explicit provider/processing/ambiguity/failure wording. This
  is automatic presentation only; no cover checkbox, scan-image fallback, or
  provider-selection mutation was added.
- RED-first evidence failed the ISBN-only request, ISBN-over-title/author
  ranking, thumbnail-only decoder, and incomplete/generic sheet expectations.
  Focused green Jest passed **3 suites / 74 tests**. Wider metadata regression
  passed **10 suites / 133 tests**; affected Unit 6G UI regression passed **6
  suites / 82 tests** with pre-existing VirtualizedList `act(...)` warnings and
  `--forceExit`; TypeScript `--noEmit`, `git diff --check`, and Phase 9
  continuity passed.
- No migration, Supabase/database/Storage mutation, provider call, credential
...
- Created one forward repository-only migration, M51, with preflight rejection
  of eligible NULL/out-of-range/duplicate orders and more than three eligible
  rows per inventory. Existing `UNIQUE(inventory_id,public_order)` plus the
  `1..3` check remains the bounded cardinality mechanism.
- Replaced the link guard and added an asset-lifecycle BEFORE guard so either
  transition into public eligibility requires a unique non-null order `1..3`.
  Pending/rejected links and private/staging assets remain nullable.
- Q10 now uses the shared public-media predicate, `ORDER BY public_order,id`,
  and `LIMIT 3`; strict client schemas enforce gallery max three and order 1–3.
- Verification actually run: U8B PGlite `63/63`; U8C PGlite `8/8`;
  Marketplace Jest `140/140`; M50/M51 migration contracts `6/6`; U8B and U8C
  disposable real-PostgreSQL acceptance sentinels PASS; `tsc --noEmit` and web
  export PASS; repository diff and Phase 9 continuity checks PASS.
- Prepared logical commits: `30b5b8b` legacy RPC security, `05a8b12`
  bookstore-first backend/M49-M51, and `baf5e4f` client Q08-Q10 integration.
- No connected database/Storage mutation, migration-history repair, Vault
  provisioning, migration application, deployment, commit, or push occurred.

### 2026-08-20 — U8B bounded corrective scope locally complete

- Scope was limited to the three confirmed U8B corrections: authoritative
  provider/approved-media provenance for cover precedence, effective relevant
  policy fingerprint binding for Q08 cursors, and bounded malformed-cursor
  error mapping. No U8C client/storefront work was started.
- Production migration: `supabase/migrations/20260818000049_marketplace_phase9_bookstore_first_discovery.sql`.
  Test support: `phase9BookstoreFirstDiscoveryTestHelpers.mjs`, the updated
  U8B integration suite, and disposable PostgreSQL SQL/bootstrap/PowerShell
  runner under `supabase/tests/phase9/`.
- Verification actually run: focused regressions `3/3` and cover fallback
  `4/4`; full U8B integration `63/63`; disposable real-PostgreSQL acceptance
  through `npm.cmd run test:phase9:u8b:postgres`, including the sentinel
  `U8B_REAL_POSTGRES_ACCEPTANCE_PASS`.
- M49 was applied only to fresh disposable local PostgreSQL/PGlite databases.
  No live Supabase migration, migration-history repair, Vault secret, Storage,
  business-row, deployment, or external-service mutation occurred.
- Residual gate: independent U8B re-review and, if approved separately,
  migration-history reconciliation, dedicated Q08 Vault provisioning, and
  live Supabase verification. M49 application and U8C remain out of scope.

### 2026-08-20 — Unit 8 Marketplace SDD freeze

- Created and froze the authoritative Unit 8 SDD under `work-units/`; no
  source, test, migration, database, Storage, deployment, fixture, or business-
  data change was made.
- M47/M48 is recorded as the satisfied legacy-RPC security prerequisite. The
  safe v2 customer path remains the current implementation.
- U8B/U8C/U8D are future handoff slices only. The known local/live migration-
  history mismatch must be reconciled before any future Unit 8 migration, and
  no such reconciliation or migration work occurred here.
- Documentation verification passed: all 22 required SDD sections are present;
  the 462-line file received the validator's cohesion assessment (no split
  advisory); requirement definitions/traceability and local links pass;
  `PHASE9_CONTINUITY_CHECK=PASS`; and repository `git diff --check` passes.

### 2026-08-17 — Legacy Marketplace RPC M47/M48 security correction PASS

- Pre-change live inspection confirmed exact signatures
  `phase9_storefront_catalogue(uuid,integer,jsonb)` and
  `phase9_listing_detail(uuid)`, postgres ownership, `SECURITY DEFINER`,
  `search_path=""`, `SETOF phase9_public_listing_projection` returns, and
  `inventory_id` in the legacy row shape. The client uses only the v2 JSON path.
- Repository/runtime dependency review found no caller. This includes all
  source/worker/script/test-runtime search results, all 12 live Edge Function
  readbacks, and the live `pg_proc` function-definition scan. M48 intentionally
  restores only trusted `service_role` compatibility after M47's immutable
  broad revoke.
- M47 is live once as `20260817073341`; M48 is live once as `20260817075825`.
  Final legacy ACLs are `{postgres=X/postgres,service_role=X/postgres}`; effective
  `anon`/`authenticated` `EXECUTE` is false and service-role `EXECUTE` is true.
- Explicit `SET ROLE` probes: anon/authenticated calls to both legacy RPCs and
  direct projection-view reads fail with `42501`; service-role detail execution
  succeeds and returns the trusted legacy row; v2 anon/auth search and detail
  succeed with allowlisted JSON and no `inventory_id`.
- Focused static/client Jest is 3 suites/15 tests green. Post-M48 read-only
  counts are store inventory `10`, marketplace listings `9`, public projection
  `9`, Storage buckets `10`, and Storage policies `19`. No Unit 8 implementation,
  business-data/Storage mutation, Edge redeploy, or unrelated grant/API change.
...
| Wake dispatcher | [Automatic worker wake dispatcher](../work-units/automatic-worker-wake-dispatcher-sdd.md) | `live_active_m56_local_unapplied` | M36 is live exactly once and M40 added publication retry; cron remains active. Local M56 changes only metadata payload to 15, passes structural/PGlite review, and remains unapplied pending worker-first canary and separate application authority. |
...
| `20260911000060_marketplace_phase9_duplicate_confirmation.sql` | **applied live as `20260912072815`** on `Bookconnect_reactexpo` / `ahntbtktjjmvfosgkmgn`; exact artifact checksum remains unavailable from connected readback | Bounded correction and disposable PostgreSQL verification on `codex/phase9-duplicate-confirmation`, followed by authorized development rollout to `Bookconnect_reactexpo` / `ahntbtktjjmvfosgkmgn`; M52–M59 source remained unchanged | Adds the duplicate confirmation state/table, canonical-only partial uniqueness, trigger relationship checks, private resolvers plus service-role-only public delegates, exact private-object proof, and pending-confirmation cleanup/dispatch fences | Local Jest 181/181, PGlite 5/5, TypeScript PASS; PostgreSQL 18.4 independent-connection concurrency and U8B regression PASS. Live duplicate Proceed created exactly one new vision job and two candidates with no inventory write; Owner Edge v11 rendered both batch-review cards. M60 was applied before the later correction. |
| `20260913000061_marketplace_phase9_representative_edition_cover.sql` | **applied live as `20260913111342`** on `Bookconnect_reactexpo` / `ahntbtktjjmvfosgkmgn` | Exact-project and artifact-hash preflight PASS; applied once after M60 | Adds the private representative-cover sidecar, checked private inventory field, service-only write delegate, RLS, immutable/copy triggers, and Owner batch projection; exact/public cover and duplicate handling remain unchanged | M61 contract/integration and readback PASS; zero sidecar/inventory rows; Owner Edge v12 active and source-matched; worker/client deployment and connected no-cover proof remain pending |
| `20260913000062_marketplace_phase9_representative_cover_detail_projection.sql` | **applied live as `20260913162154`** on `Bookconnect_reactexpo` / `ahntbtktjjmvfosgkmgn` | Exact-project and artifact-hash preflight PASS; applied once after M61 | Adds only the postgres-owned private projection helper and replaces authenticated Owner standalone detail/review-save functions so M61 provenance is available consistently; no table/column/data/Storage/public projection change | Migration contract 4/4; post-apply helper/ACL readback PASS; authenticated Owner entrypoints present, API roles denied helper access, and sidecar/inventory counts zero; matching runtime deployment and connected proof remain pending |
