...
| Unit 7C Owner command/read boundary | Live M42 still has C22/C24/C25 and no frozen Store View aggregate. Local M43 adds controlled exact-versioned Save, stock v2, and Store View page/detail RPCs; local M44 adds authenticated page v2 with authoritative server-side filters before keyset pagination and actor/store/filter-bound cursors. | WU1/WU2A local contracts are complete. Media, client cutover, and connected M43/M44 application remain separate work/gates. |
