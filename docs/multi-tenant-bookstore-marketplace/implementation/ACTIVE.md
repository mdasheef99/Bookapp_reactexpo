...
- **Latest runtime state:** M41 was accepted and M42 is applied exactly once. Owner Edge v7 and the Render publication worker remain live/ready; Unit 7B live proof is PASS and merged into `main` at `53edbddc9c5417b34cb169599e8282b162e183b3`. Unit 7C WU5 Owner Store View cutover is locally complete but uncommitted on `codex/unit7c-wu5-store-view-cutover`; local M43/M44/M45 candidates remain unapplied and no M46 exists.
...
- **Current work-unit plan:** WU5 exposes the existing Store View list as a primary Owner tab, keeps Inventory as intake/review/recovery context, demotes Storefront to Store Profile compatibility routing, and hands successful Add responses to Store View by returned `inventoryId`. Focused Owner/Inventory/Store View coverage is 10 suites/53 tests; local in-app browser navigation and mobile viewport checks pass; Expo web export passes. The Playwright CLI is `NOT_RUN_ENVIRONMENT` because Node cannot `lstat C:\Users\user`; no connected mutation or deployment occurred. The exact next action is separate authorization to commit the WU5 vertical. M43/M44/M45 application, deployment, push, and integrated Unit 7C review remain gated.
