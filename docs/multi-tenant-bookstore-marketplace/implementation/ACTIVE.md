...
- **Current work-unit plan:** Unit 7A create-only inventory commit design is frozen. Each explicit eligible candidate commit creates exactly one new private inventory row from the current server-held saved review; duplicate lookup/merge/increment/manual-match/keep-separate behavior is excluded, and publication is Unit 7B. Historical Unit 6 duplicate controls are superseded for this path and must be non-actionable/non-blocking before enablement. The design requires a forward migration and is ready for separately authorized load-bearing red tests; no Unit 7A implementation, test execution, migration creation/application, or external mutation is authorized by the planning checkpoint. Unit 6 automatic/functional PASS and its accepted deferred native validation debt remain unchanged.
