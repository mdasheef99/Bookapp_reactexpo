...
- **Current work-unit plan:** Unit 7C WU5/M46 and the resumed connected canary are PASS. Authenticated Browser → existing Owner Edge v8 → DB evidence covered private-only Save, stale Save rejection, stock `1→0→1`, Pause → Republish, approved media ADD, history/privacy checks, and Store View/Inventory/Store Profile routing. Final readback is inventory version `6`, publication intent version `4`, one stable active listing, four revisions (including the preserved historical false revision), nine audit/events, and one approved public media link. No repair/reset, additional migration, or Edge redeploy occurred. The current exact action is to publish this branch and merge it into `main`; after that gate, no further Unit 7C connected mutation is authorized without separate approval.
