...
- **Current work-unit plan:** `phase9_metadata_retry_provider_attempt_correction` is locally complete, independently `APPROVED`, and published as correction commit `8c55fea` on `codex/phase9-metadata-retry-correction`. Exact-project read-only verification confirms M32-M37 are live exactly once, ending at `20260810105517 marketplace_phase9_owner_discovery_scope_correction`. Local forward M38 exposes the selected physical provider call's originating claim attempt through metadata context v2; the worker reuses retryable finalized evidence only for the same claim and performs fresh egress on later claims. Focused metadata Jest is 148/148, the exact Google Books HTTP 503-to-200 structural regression passes, full Phase 9 PGlite is 242/242, and the metadata build, entrypoint smoke, and deployment-runtime validation pass. M38 application, deployment, live-job mutation, and every media, vision, query-policy, Unit 7, inventory, listing, or product-publication change remain prohibited and require a separately authorized session.
