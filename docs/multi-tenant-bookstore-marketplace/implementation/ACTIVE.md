...
- **Current work-unit plan:** [Phase 9 Unit 6 pre-main integration reconciliation](./phase-9-image-inventory/trackers/28-pre-main-integration-reconciliation.md) is the active Git-only unit. The complete current M32/M33/M35/Unit 6/runtime source is being packaged with the reviewed M36 dispatcher and the authoritative compact-Gemini lineage into one clean `main` candidate. M36 remains local/unapplied. Supabase, Vault, Cron, Render, worker/provider, Storage, live-image, duplicate-replay, Unit 7, inventory, listing, and publication mutations remain prohibited.
