...
- **Current work-unit plan:** Unit 7A create-only inventory commit is implemented at reviewed SHA `e2437f18e9489b1e03c27858342b44737522d4e8`, and M39 is live exactly once as `20260812003419 marketplace_phase9_create_only_inventory_commit` on `Bookconnect_reactexpo` / `ahntbtktjjmvfosgkmgn`. The post-M39 evidence commit is pushed. The sole authorized `phase9-owner-ingestion` deployment attempt failed before activation because `domain/validation.ts` transitively imports `../contracts/registers` without `.ts`; no retry or source repair was attempted, version 3 remains ACTIVE, and pre/post business state is identical. A reviewed source correction and a new deployment authorization are required before controlled live Add-to-Inventory/exact replay. Unit 7B, Unit 7C, and merge to `main` remain separately gated.
