...
- **Current work-unit plan:** [Phase 9 Unit 6 pre-main integration reconciliation](./phase-9-image-inventory/trackers/28-pre-main-integration-reconciliation.md) is the active Git-only unit. An isolated candidate has been assembled from fresh `origin/main` at merge checkpoint `8e327d7`, preserving exact M36 ancestry while restoring unrelated and superseded tree content to main's authoritative versions. Candidate verification and independent final approval remain before normal `origin/main` push. M36 remains local/unapplied. Supabase, Vault, Cron, Render, worker/provider, Storage, live-image, duplicate-replay, Unit 7, inventory, listing, and publication mutations remain prohibited.
...
- **2026-08-09 schema-free Gemini JSON correction:** Gemini now receives JSON MIME mode and the user's flat `vision` prompt with no provider response schema; BookConnect keeps strict local validation and server-owned M18/M19 mapping. Bounded capture returned eight observations and proved the exact mismatches: provider `image_outcome: success` and one null `detected_language`. Red-first reproduced the failure; BookConnect now normalizes only those values to canonical `analyzed` and `und`. Five exact-path Jest suites are 60/60 and the vision worker builds. The approval boundary blocked the separate final proof transmission, so it remains pending explicit approval. M34, push, corrected deploy, jobs, metadata, Owner UI continuation, inventory, and publication remain untouched.
