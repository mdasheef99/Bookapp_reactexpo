...
- **Current work-unit plan:** M32 is live exactly once as `20260808020404` on verified project `ahntbtktjjmvfosgkmgn`. The local metadata runtime fails closed unless `SUPABASE_URL` is the exact approved HTTPS origin, metadata has no media/vision peer fingerprint, and the bounded invoker supports `metadata`. Focused changed-scope Jest is 84/84 and the metadata worker builds. Exactly one process-only Google Books adapter request returned HTTP 200; the credential was accepted, bounds/decoder/normalization passed, and the provider-neutral outcome was `no_acceptable_match`. No database/provider-attempt persistence occurred and the temporary harness was removed. A 2026-08-09 sanitized JWT inspection and authenticated read proved that the inherited service-role key belongs to and is accepted by the exact project; only the inherited `SUPABASE_URL` is foreign and must be overridden for the worker process. `CONTROLLED_ONE_CANDIDATE_METADATA_VERTICAL_PROOF` still requires separate authorization and a fresh process-only Google Books key after revocation of the chat-exposed temporary key. Gemini, scheduler/dispatch, deployment, Unit 7, inventory, and publication remain unchanged/excluded.
