...
- **Current work-unit plan:** [Phase 9 Unit 6 pre-main integration reconciliation](./phase-9-image-inventory/trackers/28-pre-main-integration-reconciliation.md) is the active Git-only unit. An isolated candidate has been assembled from fresh `origin/main` at merge checkpoint `8e327d7`, preserving exact M36 ancestry while restoring unrelated and superseded tree content to main's authoritative versions. Independent review required local M37 so already-live M35 remains immutable; focused Jest 5 suites/189 and Phase 9 DB continuity 241/241 pass. Final correction-only re-review remains, and push requires a new explicit user instruction. M36 remains local/unapplied and M37 is local/unapplied. Supabase, Vault, Cron, Render, worker/provider, Storage, live-image, duplicate-replay, Unit 7, inventory, listing, and publication mutations remain prohibited.
