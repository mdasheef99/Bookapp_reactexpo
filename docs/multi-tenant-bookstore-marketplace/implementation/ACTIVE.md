...
- **Current work-unit plan:** Unit 5B is independently approved/merged and fixture/mock verified only. [Unit 5C Lite](./phase-9-image-inventory/work-units/05c-lite-multilingual-search-variants-sdd.md) is an independently approved documentation candidate ready for push/merge; implementation is unstarted and requires a new session/separate authorization after merge. Current selected-language runtime, strict `p9-vision-v2`, live M01 alias schema, and positive-price publication remain unchanged. No provider, migration, UI, index, inventory, publication, or commerce activity is authorized.
