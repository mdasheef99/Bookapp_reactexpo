...
- **Latest runtime state:** M43-M48 remain live at their recorded versions, and the Unit 8 rollout is now live exactly once as M49 `20260821060156`, M50 `20260821060742`, and M51 `20260821061213`. M47 revoked all non-owner execution on the obsolete projection-row Marketplace RPCs; M48 retains the customer-role revocation while restoring only `service_role` `EXECUTE`. M49-M51 add the connected Q08/Q09/Q10 customer path, Vault-backed cursor crypto, and the bounded public-media order guards. Edge v8 remains the only deployed `phase9-owner-ingestion` version with JWT verification enabled; no Edge redeploy or unrelated service change occurred.
...
- **Current work-unit plan:** Unit 8 remains live-verified and integrated into pushed `main` at `4c1d98d`. Unit 6G Group 1 contract/persistence foundation and the bounded review-correction pass are locally complete on the approved `codex/phase9-unit6g-owner-batch-review-commit-handoff` branch/worktree. M52 now preserves the legacy v2 Close definition, uses a separate nullable-session fence, keeps automatic presentation-revision advancement, derives sources per field, and rejects null command IDs. The exact next action is independent correction-only rereview; M52 application, deployment, Git publication, and Groups 2–4 remain separately gated.
