...
> **Historical Unit 6H correction checkpoint — 2026-09-12:** The duplicate-image confirmation
> implementation was locally corrected but not rolled out at that checkpoint. It preserves the
> create-only inventory boundary: Proceed analyzes the new upload and still
> requires normal Owner review and explicit Add to Inventory; it does not merge
> existing inventory or reuse an older scan. M60 was local-only at that checkpoint
> and was subsequently applied during the authorized development rollout; M61 is
> local-only. F-01 is retracted and F-02/F-03 are corrected locally. Disposable independent-connection
> PostgreSQL verification passes; connected Edge/Storage remains a rollout gate.
> This is status/evidence only and does not
> change the canonical or inventory rules in this document.
...
Bounded cover correction P9-D86: if the selected coherent provider edition has
no cover, Owner-private review may show one separately labelled
`representative_edition` cover from a compatible alternate edition already in
the same provider response. Title, authors, ISBN, edition, and all other
selected fields remain from the selected edition; the exact selected
`cover_url` remains null, no second provider request or cross-provider stitching
is allowed, and the fallback does not enter canonical/public `cover_url`.
Provider-supplied volume/order evidence participates in the conflict check.
When a provider exposes only opaque series membership rather than a trustworthy
human series name, BookConnect does not invent that name and suppresses the
fallback for either series-bearing record.
