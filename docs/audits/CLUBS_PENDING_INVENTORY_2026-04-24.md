...
**Last reconciled with current code:** 2026-06-06
...
| 1.3 | **Reading progress & schedule UI** | `Implemented After Audit` | **Low** | Reading progress route/screen/hook/service exist. Admin schedule builder and member milestone timeline now consume `reading_schedules` and `member_reading_progress`; builder includes starter templates, YYYY-MM-DD validation, chronological due-date validation, and explicit chapter-order validation. 2026-06-06 notification routing now emits reading schedule create/update notifications and hourly milestone-due reminders. Remaining work is product-depth: multi-plan behavior. |
...
| 1.8 | **Venue browse / selection / registration screens** | `Partially Implemented` | **Medium** | Venue work has started. `src/features/venues/` now includes approved-venue browse/detail services, hooks, reusable cards, browse/detail screens, routes under Clubs, and Manage > Linked venues for club venue linking/primary venue management. Venue owner registration/CRUD, admin verification UI, map/geospatial radius UX, and exchange pickup venue selection remain pending. |
| 1.9 | **Notification preferences / inbox** | `Implemented After Audit` | **High** | Profile notification inbox/settings, push token registration, durable notification tables, `send-notification`, `wishlist-notify`, event routing, and reminder cron scaffolding are implemented and live. Push dispatch from pg_cron requires server-side `app.settings.send_notification_*` configuration so the DB can authenticate to the JWT-protected Edge Function. |
...
| 2.10 | **Notification service (Expo push token + send)** | `Implemented After Audit` | **High** | `src/features/notifications/` now provides inbox/preference/token services and hooks. `send-notification` is deployed live for Expo push delivery. `wishlist-notify` is deployed live as a cron-secret-gated service-role backfill/manual trigger, while normal wishlist matches are also created by DB trigger. |
| 2.11 | **Venue service** | `Partially Implemented` | **Medium** | `src/features/venues/` now provides approved venue browse/detail reads. Clubs services/hooks support linked venue reads plus add/remove/set-primary mutations. Standalone venue owner CRUD, admin verification, and geo-radius search remain pending. |
...
| 3.4 | **`send-notification` Edge Function** | `Implemented After Audit` | **High** | Deployed live with JWT verification. Sends pending Expo push delivery rows, marks sent/failed status, and records provider message/error details. Scheduled dispatch requires server-side DB settings for the function URL/bearer/optional cron secret. |
| 3.5 | **`wishlist-notify` Edge Function** | `Implemented After Audit` | **High** | Deployed live with JWT verification and a required `WISHLIST_NOTIFY_CRON_SECRET` gate. Calls `notify_wishlist_matches()` for safe wishlist-match backfill/manual runs; new active listings are also covered by `route_wishlist_listing_match_notification`. |
...
| 4.2 | **Downgrade grace-period cron / pg_cron** | `Implemented After Audit` | **High** | Live Supabase now includes `club_downgrade_grace_events`, `process_club_downgrade_grace_period()`, `pg_cron` 1.6.4, and the daily `club-downgrade-grace-period` job. 2026-06-06 update adds downgrade warning/remediation notifications and a deadline-near reminder path. |
...
| 4.5 | **Automated cleanup of expired `club_member_actions`** | `Implemented After Audit` | **Low** | Live Supabase now has `cleanup_expired_club_member_actions()` and the `cleanup-expired-club-member-actions` pg_cron job. MCP extension verification on 2026-06-06 confirms `pg_cron` 1.6.4 is installed. |
...
| 6.4 | **Reading progress / schedule spec** | `Partially Implemented` | **Low** | Basic reading status, schedule builder, milestone timeline, explicit chapter-order validation, schedule create/update notifications, and milestone-due reminders are implemented. Product spec still needs to define multi-plan behavior. |
...
2. Keep push dispatch settings monitored: `send-notification` is live, but DB cron dispatch requires configured server-side function URL/bearer/optional cron secret.
3. Keep notification delivery monitoring/retry policy on the ops backlog.

### Immediate non-chat task order
1. Add schema-backed author experiences such as AMA/signed-edition workflows.
2. Expand reading schedule product depth: multi-plan behavior.
3. Define exact admin lifecycle automation policy beyond conservative remediation: no automatic successor selection is enabled; excess clubs are archived after the grace window unless the user/admin manually reduces count first.
4. Configure and monitor authenticated push dispatch settings for the `notification-push-dispatch` pg_cron handoff.
5. Decide whether moderation needs durable resolution notes or a stricter RPC-backed punitive workflow policy.
6. Monitor expired `club_member_actions` cleanup and downgrade grace cron behavior after scheduled runs.
7. Continue venue frontend Phase 1 follow-through; venue owner/admin/geospatial flows remain out of scope.
...
2. Venue frontend module beyond current Clubs-first browse/detail/linking phase
3. Reading schedule product depth (multi-plan behavior)
...
1. Add delivery receipt reconciliation/retry policy for Expo push notifications.
2. Configure server-side push dispatch settings for authenticated cron invocation if not already set in production.
