...
**Purpose:** Executes atomic credit transfer when transaction reaches COMPLETED state. Handles hold release, credit deduction, and book ownership transfer.

**Dependencies:**
- Supabase service role key
- Access to `transactions`, `credit_events`, `user_books`, `user_credit_balances` tables
...
**Key Logic:**
- Insert 3 credit events atomically:
  1. `lend_completed` (+1 credit to lender)
  2. `borrow_spent` (-1 credit from borrower)
  3. `hold_released` (release held credit)
- Transfer book ownership in `user_books` (lender → borrower)
- Update transaction status to `completed`
- Trigger rating prompts for both users
