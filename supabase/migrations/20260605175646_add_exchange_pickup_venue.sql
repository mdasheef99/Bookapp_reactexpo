...
ADD COLUMN IF NOT EXISTS pickup_venue_id uuid REFERENCES public.venues(id);

CREATE INDEX IF NOT EXISTS idx_transactions_pickup_venue
ON public.transactions(pickup_venue_id)
WHERE pickup_venue_id IS NOT NULL;

DROP FUNCTION IF EXISTS public.request_transaction(uuid, uuid, text, text, uuid);
