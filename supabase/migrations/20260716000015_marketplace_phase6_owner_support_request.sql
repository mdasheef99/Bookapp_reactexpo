-- Phase 6 Unit 8C: non-transitioning Owner platform-support request.
BEGIN;
CREATE FUNCTION public.request_platform_support(p_request_id UUID,p_expected_version INTEGER,
...
BEGIN
 ...
 ELSE
  INSERT INTO public.event_action_tasks(event_id,store_id,assigned_role,status,entity_type,
   entity_id,task_type,next_attempt_at,dedupe_key,support_version)
  VALUES(v_event,v_request.store_id,'support_agent','open','store_order_request',v_request.id,
   'platform_support_request',transaction_timestamp(),'support:'||v_request.id,1)
...
END;$$;
REVOKE ALL ON FUNCTION public.request_platform_support(UUID,INTEGER,TEXT,TEXT,TEXT,UUID) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.request_platform_support(UUID,INTEGER,TEXT,TEXT,TEXT,UUID) TO authenticated,service_role;
COMMIT;
